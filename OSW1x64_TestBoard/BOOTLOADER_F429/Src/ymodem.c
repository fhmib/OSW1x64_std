...
/**
  ...
  */
COM_StatusTypeDef Ymodem_Receive (uint32_t addr, uint32_t *p_size )
{
...
COM_StatusTypeDef Ymodem_Transmit (uint8_t *p_buf, const uint8_t *p_file_name, uint32_t file_size)
{
  ...
  {
    /* Send Packet */
    HAL_UART_Transmit(&huart3, &aPacketData[PACKET_START_INDEX], PACKET_SIZE + PACKET_HEADER_SIZE, NAK_TIMEOUT);
    ...
    /* Wait for Ack and 'C' */
    if (HAL_UART_Receive(&huart3, &a_rx_ctrl[0], 1, NAK_TIMEOUT) == HAL_OK)
    {
      if (a_rx_ctrl[0] == ACK)
      {
        ack_recpt = 1;
      ...
      {
        if ((HAL_UART_Receive(&huart3, &a_rx_ctrl[0], 1, NAK_TIMEOUT) == HAL_OK) && (a_rx_ctrl[0] == CA))
        {
          HAL_Delay( 2 );
  ...
  while ((size) && (result == COM_OK ))
  {
    ...
    while (( !ack_recpt ) && ( result == COM_OK ))
    {
      ...
      HAL_UART_Transmit(&huart3, &aPacketData[PACKET_START_INDEX], pkt_size + PACKET_HEADER_SIZE, NAK_TIMEOUT);
      ...
      /* Wait for Ack */
      if ((HAL_UART_Receive(&huart3, &a_rx_ctrl[0], 1, NAK_TIMEOUT) == HAL_OK) && (a_rx_ctrl[0] == ACK))
      {
        ack_recpt = 1;
        if (size > pkt_size)
        {
